...
const StoreSettingsPage: React.FC<Props> = ({ settings, products, onSave }) => {
  ...
  const handleBackup = async () => {
    ...
    setIsExporting(true);
    try {
      const [pRes, cRes, oRes, sRes, wRes] = await Promise.all([
        supabase.from('products').select('*'),
        supabase.from('categories').select('*'),
        supabase.from('orders').select('*'),
        supabase.from('settings').select('*'),
        supabase.from('waitstaff').select('*')
      ]);
  ...
  const handleRestore = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (data.categories.length > 0) await supabase.from('categories').upsert(data.categories);
        if (data.products.length > 0) await supabase.from('products').upsert(data.products);
        if (data.settings.length > 0) await supabase.from('settings').upsert(data.settings);
        if (data.waitstaff.length > 0) await supabase.from('waitstaff').upsert(data.waitstaff);
        if (data.orders.length > 0) await supabase.from('orders').upsert(data.orders);
